...
export default class APIService {
  ...
  async addItemToCart(userId: number, itemId: number) {
    return this.api
      .post(`/${userId}/shopping_cart/`, JSON.stringify({ itemId: itemId }))
      .then((response) => response)
      .catch((error) => error.response);
  }
  ...
  async removeItemFromCart(userId: number, itemId: number) {
    return this.api
      .delete(`/${userId}/shopping_cart/`, { data: { itemId: itemId } })
      .then((response) => response)
      .catch((error) => error.response);
  }
  ...
  async finishOrder(userId: number, price: number) {
    const orderId = (await this.api.get(`/${userId}/shopping_cart/orderId`))
      .data.orderId;
    return this.api.put(`/orders/${orderId}/`, JSON.stringify({ price }));
  }
  ...
  async cancelOrder(
    ...
    return this.api.put(`/clients/${clientId}/orders/${orderId}/cancellation`, {
      password: password,
      reason: reason,
    });
  }
  ...
  postLoginClient(email: String, password: String) {
    return this.api.post("/clients/login", {
      email: email,
      password: password,
    });
  }
