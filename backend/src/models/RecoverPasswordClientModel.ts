...
class RecoverPasswordClientModel{
  static async getByEmail(email: string){
      const client = await prisma.client.findUnique({where: {email : email}}); //verifica se o e-mail existe no banco
  
      if(!client) { throw new NotFoundError('Email not found.');} // se não existir jogar
  ...
  static async verifyCode (email: string, code: string){
    ...
    if(!client) { throw new NotFoundError('Email not found.');} // se não existir jogar
    if(client.code !== code) { throw new AuthenticationError('Incorrect code.')};
  }
...
export default RecoverPasswordClientModel;